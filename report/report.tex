...
Version 2 is a clear improvement over the initial dense baseline because legal chunk metadata and hybrid reranking make source selection much stronger. The remaining weakness is exact article retrieval, not answer fluency. For legal RAG, topical similarity is insufficient; the system must retrieve the precise rule before generation starts. The next step should therefore be a real sparse component, such as BM25 over all chunks, combined with dense retrieval and stronger boosts for article titles and article numbers. The evaluation should also move away from old fixed-chunk IDs and use source, article, and phrase expectations as the stable metrics.

Overall, Zakonodajko demonstrates a reproducible RAG loop for Slovenian tax law, from local legal documents through chunking, FAISS indexing, retrieval, generation, and structured evaluation. The corrected results show strong source-level progress, but article hit@3 and manual correctness are still too low for dependable legal question answering.
