#!/usr/bin/env bash
# Compare Mistral and GaMS-9B as RAG answer generators on final_dataset.
#
# Submit from the project root:
#   sbatch slurm/compare_mistral_gams9_final_v100.sh
#
# Optional overrides:
#   LIMIT=5 sbatch slurm/compare_mistral_gams9_final_v100.sh
#   REBUILD_INDEX=1 sbatch slurm/compare_mistral_gams9_final_v100.sh
#   sbatch --constraint=h100 --mem=80G --time=01:00:00 slurm/compare_mistral_gams9_final_v100.sh

#SBATCH --job-name=rag-mistral-gams9
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  ...
  echo "Use: sbatch slurm/compare_mistral_gams9_final_v100.sh"
  exit 1
fi
...
TIMINGS_TSV="logs/model-compare-final-mistral-gams9-timings-${RUN_ID}.tsv"
